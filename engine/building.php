...
class Building {	
	...
	private function upgradeBuilding($id) {
		...
		if($this->allocated < $this->maxConcurrent) {
			...
			if($database->addBuilding($village->wid,$id,$village->resarray['f'.$id.'t'],$loop,$time)) {
				...
				$logging->addBuildLog($village->wid,$this->procResType($village->resarray['f'.$id.'t']),($village->resarray['f'.$id]+1),0);
				if($id >= 19) {
					header("Location: dorf2.php");
				}
				else {
	...
	public function getTypeLevel($tid) {
		...
		if($element >= 2) {
			if($tid <= 4) {
				...
				for($i=0;$i<=$element-1;$i++) {
					if($village->resarray['f'.$keyholder[$i]] != $this->getTypeMaxLevel($tid)) {
						$target = $i;
					}
				}
			}
	...
	public function resourceRequired($id,$tid) {
		$name = "bid".$tid;
		...
		$iron = $dataarray[$village->resarray['f'.$id]+1]['iron'];
		$crop = $dataarray[$village->resarray['f'.$id]+1]['crop'];
		$pop = $dataarray[$village->resarray['f'.$id]+1]['pop'];
		if ($tid == 15) {
			if($this->getTypeLevel(15) == 0) {
				$time = round($dataarray[$village->resarray['f'.$id]+1]['time']/ SPEED *5);
			}
			else {
				$time = round($dataarray[$village->resarray['f'.$id]+1]['time'] / SPEED);
			}
		}
		else {
			if($this->getTypeLevel(15) != 0) {
				$time = round($dataarray[$village->resarray['f'.$id]+1]['time'] * ($bid15[$this->getTypeLevel(15)]['attri']/100)  / SPEED);
			}
			else {
				$time = round($dataarray[$village->resarray['f'.$id]+1]['time']*5 / SPEED);
			}
		}
		$cp = $dataarray[$village->resarray['f'.$id]+1]['cp'];
